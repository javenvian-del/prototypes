...
0. **首次触发先介绍能力，再获取本次表格链接；之后才运行浏览器或登录检查。** 每次新对话首次使用时，简要说明下面的能力与脚本执行方式；同一对话后续不重复介绍。用户只输入技能名称，或尚未提供/明确确认本次表格链接时，使用以下开场：

   > 我可以根据飞书表格，在用户增长运营平台批量创建埋点，并将事件名称和事件标识自动回写到表格中。只配置基础信息，已有结果自动跳过，元素名称中的特殊字符自动清理。
   >
   > 实际读取、创建和回写都由独立脚本执行，我负责必要的监督和异常处理。通过减少逐行模型参与和日志读取，节省 token 消耗；运行中只在异常或任务完成时通知你。执行前会检查两端登录，并确认本次处理的行区间。
   >
   > 请给我批量创建埋点的表格链接，也可以告诉我“查看模板”，我会给您批量创建埋点的表格模板。

   面向首次使用者的能力介绍使用“回写到表格中”，不出现I/J等列号；列映射仅用于内部执行或用户主动询问具体写入位置时。

   然后等待回答，不先读取历史配置、不启动浏览器、不询问行区间。即使本机保存过表格链接或登录会话，也不能据此默认选择本次表格。用户已在本次对话提供链接时，首次仍简要介绍能力和脚本执行方式，但省略索要链接，进入下一步；明确继续本次对话已确认的同一表格时直接继续。节省token表述指减少模型参与，不承诺零消耗或未经测量的节省比例。
